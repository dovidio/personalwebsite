...
After building the first prototype, I've realized that it would be much cooler if user wouldn't have to manually add a file, but could just search the songs inside the app. Luckily, Apple provides [https://developer.apple.com/musickit](Music Kit), a library that allows to programmatically interact with the Apple Music catalog. I've decided to use this library, and since it currently [https://developer.apple.com/forums/thread/698902](does't work correctly on Mac), I've pivoted to iOS and iPadOS.
